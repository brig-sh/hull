...
//
// It is the only pass that holds the tar header, and therefore the only place
// the image's true ownership can still be read. archive.Apply keeps running
// first regardless: it contributes something checkpoint/restore depends on,
// which is not yet pinned down, so this adds a pass rather than replacing one.
const recordsOwnership = true
...
// GuestAttr returns the ownership and mode recorded for path by the unpack,
// and whether there was a record at all.
//
// The absence of one is normal and not an error: it means the file was not
// unpacked from an image -- something the host put there, or something a
// previous hull wrote before it recorded anything. The caller decides what
// that should look like in the guest.
func GuestAttr(path string, symlink bool) (mode, uid, gid uint32, ok bool) {
	value := make([]byte, 12)
	var n int
	var err error
	if symlink {
		n, err = unix.Lgetxattr(path, guestAttrXattr, value)
	} else {
		n, err = unix.Getxattr(path, guestAttrXattr, value)
	}
	if err != nil || n != 12 {
		return 0, 0, 0, false
	}
	return binary.LittleEndian.Uint32(value[0:4]),
		...
		binary.LittleEndian.Uint32(value[8:12]), true
}
