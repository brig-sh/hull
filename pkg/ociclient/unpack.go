...
// refuseUnroutedTypes drops the tar entry types containerd's pass must never
// be allowed to create.
//
// The hard link is the one that matters. containerd resolves a link target's
// parent through fs.RootPath but leaves the base component alone on purpose --
// "Allow hardlink to the softlink, not the real one" -- which is right on
// Linux, where link(2) does not dereference. On darwin link(2) follows the
// symlink, so a layer that plants a symlink to a host file and then hard-links
// through it gets a name inside the rootfs that IS the host file's inode.
// containerd then applies the header to it: lchown, the PAX xattrs, lchmod,
// chtimes. That is an arbitrary chmod -- setuid included -- on any file the
// user owns, and the pull reports success.
//
// os.Root cannot see this. It guards path resolution, and a hard link is not a
// resolution; every containment fix in this file sits downstream of the pass
// that already did the work. The evasion is that we resolve a link source
// lexically (path.Clean) while containerd resolves it for real, so the Lstat
// guard below inspects a different file than the one containerd linked.
//
// Filtering costs nothing: our own extractor handles hard links (with a copy
// fallback) and refuses FIFOs and device nodes, so containerd contributes
// nothing here that we do not do ourselves, more carefully. FIFOs are included
// because mkfifo needs no privilege and one left at a path we later read --
// /etc/passwd, urunc.json, a kernel -- blocks in open(2) forever, which no
// amount of path containment prevents.
func refuseUnroutedTypes(h *tar.Header) (bool, error) {
	switch h.Typeflag {
	case tar.TypeLink, tar.TypeFifo, tar.TypeChar, tar.TypeBlock:
		return false, nil
	}
	return true, nil
}
...
// stays until that is understood -- but it stays filtered.
func unpackLayer(layer v1.Layer, rootfsDir string, index int, p *Progress) error {
	...
	_, err = archive.Apply(ctx, rootfsDir, src, archive.WithFilter(refuseUnroutedTypes))
	...
	//
	// The substring backstop that used to sit alongside these is gone. It read
	// the formatted message, and containerd puts the failing entry's name in
	// nearly every error it returns -- so an image shipping a file literally
	// named "operation not permitted" made any containerd failure, including
	// its own containment refusals, classify as the expected EPERM and be
	// discarded. The image chose whether we noticed. errors.Is is the whole
	// test now: containerd wraps the failing os.Lchown with %w, so it reaches
	// the syscall errno, and an EPERM formatted with %v somewhere in a
	// dependency now fails the pull loudly rather than passing quietly.
...
func resolveImageUser(rootfsDir, spec string) (uint32, uint32, string, error) {
	...
	if hasGroup {
		if n, err := strconv.ParseUint(groupPart, 10, 32); err == nil {
			...
			fields := fileLookup(rootfsDir, "etc/group", 0, groupPart, 4)
			if fields == nil {
...
func passwdLookup(rootfsDir string, keyIdx int, key string) []string {
	return fileLookup(rootfsDir, "etc/passwd", keyIdx, key, 7)
}

// fileLookup scans a colon-separated database inside the image rootfs for a
// line whose field `keyIdx` equals `key` and has at least minFields fields.
//
// Read through an os.Root, like every other reader of unpacked image content.
// This one was a plain os.ReadFile on a joined path, which is the whole escape:
// the image plants etc/passwd as a symlink to a host file, and field 6 of
// whatever that file happens to contain lands in the OCI spec's HOME= and is
// handed to the guest. The user part is image-controlled too, so the image
// chooses both the file and the line.
//
// The regular-file check is separate from containment and not covered by it. A
// FIFO here is opened, not resolved, so os.Root admits it and the read blocks
// forever; refusing anything that is not a regular file is what actually stops
// that.
func fileLookup(rootfsDir, rel string, keyIdx int, key string, minFields int) []string {
	...
	if fi, lerr := root.Lstat(rel); lerr != nil || !fi.Mode().IsRegular() {
		if lerr == nil {
			log.Debugf("ociclient: refusing to read %s: not a regular file (%v)", rel, fi.Mode().Type())
		}
		return nil
	}
...
// is relaxed first, and put straight back afterwards.
//
// The restore is not optional, for the same reason createForWrite restores:
// nothing else is guaranteed to. applyDirModes only corrects a directory some
// later layer re-states, so an image whose 0500 /usr/lib is widened here and
// never mentioned again ships that widening into the guest rootfs as 0700 --
// a permission the image never asked for, on the one rootfs whose modes the
// guest reads straight through. Restoring the full mode rather than the
// permission bits alone also keeps setuid, setgid and the sticky bit: widening
// a 1777 /tmp and writing back only 0777 would leave every user able to delete
// every other user's files there.
//
