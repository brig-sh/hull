...
func unpackLayer(layer v1.Layer, rootfsDir string, index int, p *Progress) error {
	...
	// On macOS the manual extractor is not a fallback but the only correct
	// path: it is the one that records what each entry's ownership is inside
	// the image, which nothing on the host filesystem can express. See
	// recordGuestAttr.
	if preferManualExtract {
		if err := extractTarIgnoreChown(src, rootfsDir); err != nil {
	...
	for {
		...
		case tar.TypeLink:
			...
			if err := os.Link(source, target); err != nil {
				...
				keepAttr(false)
				if cerr := os.Chmod(target, os.FileMode(header.Mode)); cerr != nil {
