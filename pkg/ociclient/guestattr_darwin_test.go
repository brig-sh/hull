...
// vz reports the host file's mode straight to the guest, so the setuid bit has
// to be on the file itself and not only in the record hvi reads.
//
// os.Chmod loses it: an os.FileMode carries setuid at 1<<23, not at the POSIX
// 04000, so converting a tar header's raw 04755 quietly yields 0755. sudo then
// fails on vz with "must be owned by uid 0 and have the setuid bit set" even
// though the ownership is right.
func TestUnpackKeepsSetuidOnTheHostFile(t *testing.T) {
	dir := t.TempDir()
	data := attrTarball(t, []*tar.Header{
		{Name: "usr/bin/sudo", Typeflag: tar.TypeReg, Mode: 0o4755, Uid: 0, Gid: 0},
		{Name: "usr/bin/wall", Typeflag: tar.TypeReg, Mode: 0o2755, Uid: 0, Gid: 5},
	...
	st, err := os.Stat(filepath.Join(dir, "usr/bin/sudo"))
	if err != nil {
		t.Fatal(err)
	}
	if st.Mode()&os.ModeSetuid == 0 {
		t.Errorf("sudo is %v on the host: without the setuid bit vz boots a sudo that refuses to run", st.Mode())
	}
	if st.Mode().Perm() != 0o755 {
		t.Errorf("sudo perm bits are %o, want 755", st.Mode().Perm())
	}

	if st, err := os.Stat(filepath.Join(dir, "usr/bin/wall")); err != nil {
		t.Fatal(err)
	} else if st.Mode()&os.ModeSetgid == 0 {
		t.Errorf("wall is %v on the host, want the setgid bit", st.Mode())
	}
}
