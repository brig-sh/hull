...
func Fetch(ctx context.Context, ref, storeDir string) error {
	...
	// Check the signature before a single byte of kernel reaches the asset
	// directory. Doing it after the write would mean a rejected bundle still
	// replaced the kernel that boots, and the operator would be one `hull run`
	// away from the thing we just refused.
	//
	// The check is here, in Fetch, because this is the only place a bundle
	// enters the host -- `hull run`, `hull assets pull` and brig's fetch
	// callback all arrive through it.
	verifyMode := VerifyModeFromEnv()
	verdict := DefaultVerifyPolicy().VerifyBundle(ctx, ref, bundleDigest)
	if verr := verdict.Err(verifyMode); verr != nil {
		return verr
	}
	verifiedDigest := ""
	switch verdict.Outcome {
	case BundleVerified:
		verifiedDigest = verdict.Digest
		log.Debugf("%s", verdict.Message())
	default:
		// Nothing was proved, and the record will say so by leaving
		// verifiedDigest empty. It is a warning rather than a debug line
		// because the alternative is that the only trace of an unverified
		// kernel is a field in a JSON file nobody opens.
		log.Warnf("%s", verdict.Message())
	}
	...
	if err := writeProvenance(dir, ref, bundleDigest, verifiedDigest, files); err != nil {
		return err
	}
	return nil
}
