...
func (p VerifyPolicy) VerifyBundle(ctx context.Context, ref, digest string) VerifyResult {
	...
	case looksUnsigned(out):
		res.Outcome = BundleUnsigned
		res.Detail = firstVerifyLine(out, err)
	default:
		res.Outcome = BundleFailed
		res.Detail = firstVerifyLine(out, err)
	}
	return res
}
...
// looksUnsigned separates "there is no signature here" from "the signature is
// wrong". cosign exits non-zero for both, and the difference decides whether a
// boot stops, so it is read off the message cosign prints rather than guessed
// from the exit code.
func looksUnsigned(out string) bool {
