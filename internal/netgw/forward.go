...
//
// The protocol defaults to tcp, which is what every caller so far asks for.
// Both addresses must carry a port: a bare host would otherwise reach
// net.Listen as an address it parses and refuses far from here, with neither
// the flag nor the request that carried it in the message.
func (f *Forward) validate() error {
	...
	if f.Protocol != "tcp" && f.Protocol != "udp" {
		return fmt.Errorf("%w: protocol %q is not one this gateway forwards, use tcp or udp",
			ErrForwardInvalid, f.Protocol)
	}
	for _, a := range []struct{ what, addr string }{{"local", f.Local}, {"remote", f.Remote}} {
		host, port, err := net.SplitHostPort(a.addr)
		if err != nil {
			return fmt.Errorf("%w: %s address %q is not host:port: %w",
				ErrForwardInvalid, a.what, a.addr, err)
		}
		if host == "" {
			return fmt.Errorf("%w: %s address %q names no host", ErrForwardInvalid, a.what, a.addr)
		}
		n, err := strconv.Atoi(port)
		if err != nil || n < 1 || n > 65535 {
			return fmt.Errorf("%w: %s address %q does not carry a port between 1 and 65535",
				ErrForwardInvalid, a.what, a.addr)
		}
	}
...
func (s *forwards) unexpose(protocol, local string) (Forward, error) {
	f := Forward{Protocol: protocol, Local: local}
	if f.Protocol == "" {
		f.Protocol = "tcp"
	}
	f.Protocol = strings.ToLower(f.Protocol)
	...
	if err := s.fw.Unexpose(protocolOf(have.Protocol), have.Local); err != nil {
