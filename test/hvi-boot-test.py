...
# spawn (and after any image pull), so intervals from it include the VMM's
# own startup and firmware, not just the kernel. "Run /." is the init
# wrapper's exec line, where the image ships one that prints it; the token
...
if markers[b"Run /."] is not None:
    print(f"boot: vmm-spawn-to-init {markers[b'Run /.'] - origin:.2f}s (backend hvi{origin_note})")
if markers[TOKEN.encode()] is not None:
    print(f"boot: vmm-spawn-to-entrypoint {markers[TOKEN.encode()] - origin:.2f}s (backend hvi{origin_note})")
