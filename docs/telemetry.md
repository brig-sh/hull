...
| field | example | notes |
|---|---|---|
| `schema_version` | `3` | bumped on any schema change, with this page updated. 3 added the `ready` event |
...
The probe retries once a second, so `ready_ms` can be up to a second late
for every dial the guest resets before its agent listens.
