...
func execCapture(s *store.Store, instanceID string, argv, env []string, user string, timeout time.Duration) (int, string, error) {
	...
	for {
		...
		case agentproto.TypeStdout, agentproto.TypeStderr:
			// Bounded. This buffer holds whatever the guest chooses to print,
			// and the only limit on it was a deadline -- a guest streaming
			// mebibyte frames took the host to 15GB of resident memory in
			// twelve seconds. The callers here run short, known commands
			// (`ps`, a healthcheck), so anything past the cap is a guest
			// answering a question nobody asked.
			if out.Len() >= maxCaptureBytes {
				return 0, out.String(), fmt.Errorf(
					"%w: the guest sent more than %d bytes in reply to a command "+
						"that should answer in a few lines", errAgentTransport, maxCaptureBytes)
			}
			out.Write(f.Payload)
...
// maxCaptureBytes bounds a captured guest reply. The commands that use capture
// are short and known -- ps, a healthcheck, a one-shot -- so this is far past
// any honest answer and small enough that a hostile guest cannot spend the
// host's memory with it.
...
func waitHealthyExec(s *store.Store, instanceID string, hc *types.HealthCheckConfig) error {
	...
	for {
		...
		case code == 0:
			return nil
		default:
			lastErr = fmt.Errorf("healthcheck exited %d: %s", code,
				sanitizeGuestText(strings.TrimSpace(out)))
		}
...
func runServiceHooks(s *store.Store, instanceID, svcName, kind string, hooks []types.ServiceHook) error {
	for i, h := range hooks {
		...
		if code != 0 {
			return fmt.Errorf("service %q: %s[%d] exited %d: %s", svcName, kind, i, code,
				sanitizeGuestText(strings.TrimSpace(out)))
		}
	}
	return nil
...
func composeTop(_ context.Context, cmd *cli.Command) error {
	...
	for _, svcName := range services {
		...
		switch {
		case err != nil:
			// The sibling branch two lines down was sanitized in round 5 and
			// this one was not; the error it prints carries the guest's own
			// message.
			fmt.Printf("  error: %v\n", sanitizeGuestError(err))
			if firstErr == nil {
				firstErr = err
			}
		case code != 0:
			fmt.Printf("  ps exited %d: %s\n", code, sanitizeGuestText(strings.TrimSpace(out)))
			if firstErr == nil {
				firstErr = fmt.Errorf("service %q: ps exited %d", svcName, code)
			}
		default:
			// Guest-chosen bytes: this is the output of ps inside the VM.
			for _, line := range strings.Split(strings.TrimRight(out, "\n"), "\n") {
				fmt.Printf("  %s\n", sanitizeGuestText(line))
			}
		}
	}
	return firstErr
