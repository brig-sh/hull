...
func removeInstanceIn(s *store.Store, instanceID string, force bool) error {
	...
	if (state.Status == "running" || state.Status == store.StatusStarting) && state.PID > 0 {
		...
		// out.
		//
		// It must not simply be removed either. The directory about to be
		// deleted is that VM's rootfs, disk image and console log, and the name
		// about to be freed is one a live VM still answers to. So reap first,
		// using the argv recorded before the spawn, and only then delete. A
		// warning telling the operator to grep for the process left the
		// deletion happening underneath it regardless.
		if reapOrphanVMM(state) {
			log.Warnf("instance %s never got past starting; a VMM it had already spawned "+
				"was still running and has been killed before removing its directory",
				instanceID)
		} else {
			log.Warnf("instance %s never got past starting, so no pid was recorded; "+
				"removing it. No matching VMM process was found", instanceID)
		}
	}
