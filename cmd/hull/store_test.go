...
// attachTestStore creates a small sparse image and mounts it with args, so a
// test can look at how the mount really behaves rather than at what hdiutil
// was asked for. Returns the mount point and the image path.
func attachTestStore(t *testing.T, attach func(image, dir string) []string) (string, string) {
	t.Helper()
	base := t.TempDir()
	image := filepath.Join(base, "store.sparseimage")
	dir := filepath.Join(base, "mnt")
	if err := os.MkdirAll(dir, 0o700); err != nil {
		t.Fatal(err)
	}
	if err := runHdiutil("create", "-type", "SPARSE", "-fs", "Case-sensitive APFS",
		"-size", "10m", "-volname", "hull-store-test", image); err != nil {
		t.Skipf("cannot create a test disk image here: %v", err)
	}
	if err := runHdiutil(attach(image, dir)...); err != nil {
		t.Fatalf("attach: %v", err)
	}
	t.Cleanup(func() { _ = runHdiutil("detach", dir, "-force") })
	return dir, image
}

// macOS mounts disk images with ownership ignored unless told otherwise, and
// on such a volume the kernel enforces neither uid nor mode. Every 0600 hull
// writes inside the store -- state.json, urunit.conf, the container-boot
// initrd carrying the guest's environment secrets -- is then readable by any
// other local account, which is the entire access control the store has.
func TestStoreMountEnforcesOwnership(t *testing.T) {
	// The negative control: the arguments hull used to pass. If this ever
	// stops producing a noowners mount, the flag below is no longer load
	// bearing and this whole check can go.
	legacy, _ := attachTestStore(t, func(image, dir string) []string {
		return []string{"attach", image, "-mountpoint", dir, "-nobrowse"}
	})
	ignored, err := storeIgnoresOwnership(legacy)
	if err != nil {
		t.Fatal(err)
	}
	if !ignored {
		t.Skip("this host mounts disk images with ownership enforced by default")
	}

	dir, _ := attachTestStore(t, storeAttachArgs)
	ignored, err = storeIgnoresOwnership(dir)
	if err != nil {
		t.Fatal(err)
	}
	if ignored {
		t.Fatalf("%s is mounted noowners: the 0600 on everything in the store is decoration", dir)
	}
}

// Every store created before the flag existed is still mounted the old way,
// and the marker file makes ensureStore return before it reaches the attach.
// The repair path has to notice and remount, or the fix reaches only machines
// that have never run hull.
func TestEnsureStoreOwnershipRemountsAnOldStore(t *testing.T) {
	dir, image := attachTestStore(t, func(image, dir string) []string {
		return []string{"attach", image, "-mountpoint", dir, "-nobrowse"}
	})
	ignored, err := storeIgnoresOwnership(dir)
	if err != nil {
	...
	if err := ensureStoreOwnership(dir, image); err != nil {
	...
	if ignored {
		t.Fatalf("%s still ignores ownership after the repair", dir)
	}
}

// A store that is already correct must not be detached and remounted on every
// command: that would tear the volume out from under running instances.
func TestEnsureStoreOwnershipLeavesAGoodMountAlone(t *testing.T) {
	dir, image := attachTestStore(t, storeAttachArgs)
	marker := filepath.Join(dir, "canary")
	if err := os.WriteFile(marker, []byte("x"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := ensureStoreOwnership(dir, image); err != nil {
		t.Fatalf("repair on a healthy store: %v", err)
	}
	if _, err := os.Stat(marker); err != nil {
		t.Fatalf("the store was remounted needlessly: %v", err)
	}
}
