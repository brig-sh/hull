...
func TestUrunitConfIsNotGroupOrWorldReadable(t *testing.T) {
	...
	ast.Inspect(file, func(n ast.Node) bool {
		...
		// The writes go through writeIntoRootfs(rootfsDir, name, data, mode)
		// rather than os.WriteFile: the file lands in an unpacked image, and
		// the path has to be resolved under an os.Root so an image-planted
		// symlink cannot redirect it onto a host file. The mode is the last
		// argument either way.
		fn, ok := call.Fun.(*ast.Ident)
		if !ok || fn.Name != "writeIntoRootfs" || len(call.Args) != 4 {
			return true
		}
		...
		if !ok || target.Value != `"urunit.conf"` {
			return true
		}
		found++
	...
	if found == 0 {
		t.Fatal(`found no writeIntoRootfs(_, "urunit.conf", ...) calls in run.go; ` +
