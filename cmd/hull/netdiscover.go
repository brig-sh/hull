...
func injectHosts(rootfsDir string, entries []string) error {
	...
	if existing, rerr := os.ReadFile(hostsPath); rerr == nil &&
		len(existing) > 0 && !bytes.HasSuffix(existing, []byte("\n")) {
		if werr := os.WriteFile(hostsPath, append(existing, '\n'), 0644); werr != nil {
			return fmt.Errorf("failed to terminate /etc/hosts: %w", werr)
		}
	}
...
func hostsWithEntries(rootfsDir string, entries []string) ([]byte, error) {
	...
	existing, err := os.ReadFile(filepath.Join(rootfsDir, "etc", "hosts"))
	...
	for _, e := range entries {
		...
		parts := strings.SplitN(e, sep, 2)
		if len(parts) != 2 || parts[0] == "" || parts[1] == "" {
			return nil, fmt.Errorf("invalid --add-host entry %q, expected host:ip", e)
		}
		host, ip := parts[0], parts[1]
		if net.ParseIP(ip) == nil {
			return nil, fmt.Errorf("invalid --add-host entry %q: %q is not an IP address", e, ip)
		}
		fmt.Fprintf(&out, "%s\t%s\n", ip, host)
	}
