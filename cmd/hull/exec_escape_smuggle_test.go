...
func TestEscapeCannotBeSmuggledBehindAnAbortedSequence(t *testing.T) {
	// The payload is an OSC 52 QUERY, not a set.
	//
	// A set is how every TUI copies to the clipboard, and blocking it broke
	// copy/paste for real users, so it is allowed now. The query is the one
	// that matters: it makes the terminal reply with the clipboard's contents
	// into the guest's stdin. Smuggling still has to be impossible for it.
	const payload = "52;c;?"
