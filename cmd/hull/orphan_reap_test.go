...
func stubVMM(t *testing.T) (*exec.Cmd, []string, <-chan struct{}) {
	...
	// A loop, not a bare `sleep`: sh exec-optimises a single-command -c and
	// replaces its own argv, which is precisely the argv being matched on.
	args := []string{"/bin/sh", "-c", "while :; do sleep 1; done # " + marker}
