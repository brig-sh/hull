...
func (f *fakeForwards) Expose(fwd netgw.Forward) (netgw.Forward, error) {
	// Read through netgw rather than a second copy of the rule, and what it
	// returns is what gets stored. Storing the caller's forward instead let
	// this fake answer for a spelling netgw normalizes: `:8080` came back as
	// itself and a later `127.0.0.1:8080` read as a different address.
	//
	// Two calls because one will not do it. ParseTarget reads the protocol
	// and the local address, and ParseForward reads the remote -- but only
	// through a rendered spec, and Forward.String renders every protocol that
	// is not udp as tcp, so an sctp forward would come back accepted.
	protocol, local, err := netgw.ParseTarget(fwd.Protocol, fwd.Local)
	if err != nil {
		return netgw.Forward{}, err
	}
	spec := local + "=" + fwd.Remote
	if protocol == "udp" {
		spec = "udp:" + spec
	}
	parsed, err := netgw.ParseForward(spec)
	if err != nil {
		return netgw.Forward{}, err
	}
	for _, have := range f.set {
		if have.Local == parsed.Local && have.Protocol == parsed.Protocol {
			return netgw.Forward{}, fmt.Errorf("%w: %s/%s carries %s", netgw.ErrForwardExists,
				have.Protocol, have.Local, have.Remote)
		}
	}
	f.set = append(f.set, parsed)
	return parsed, nil
}

func (f *fakeForwards) Unexpose(protocol, local string) (netgw.Forward, error) {
	// netgw's own grammar, so this fake tells an argument it cannot read
	// apart from a forward nothing holds, the way the gateway does.
	protocol, local, err := netgw.ParseTarget(protocol, local)
	if err != nil {
		return netgw.Forward{}, err
	}
...
func TestForwardsAPITellsTheFailuresApart(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"a protocol this gateway does not forward", http.MethodPost, "/forwards",
			`{"protocol":"sctp","local":"127.0.0.1:1","remote":"10.87.0.2:80"}`, http.StatusBadRequest},
		{"a host named rather than addressed", http.MethodPost, "/forwards",
			`{"local":"localhost:1","remote":"10.87.0.2:80"}`, http.StatusBadRequest},
		{"a port outside the range a forward carries", http.MethodPost, "/forwards",
			`{"local":"127.0.0.1:0","remote":"10.87.0.2:80"}`, http.StatusBadRequest},
		// A DELETE the gateway cannot read is the invalid it is on POST. As a
		// 404 it reads as "that forward is gone", and a caller retries a typo.
		{"a DELETE naming a protocol this gateway does not forward", http.MethodDelete,
			"/forwards?protocol=sctp&local=127.0.0.1:1", "", http.StatusBadRequest},
		{"a DELETE naming no local address", http.MethodDelete, "/forwards", "", http.StatusBadRequest},
		{"a DELETE naming an address with no port", http.MethodDelete,
			"/forwards?local=garbage", "", http.StatusBadRequest},
