...
// Restart policies and the project supervisor (ADR-0007 section 3).
//
...
//     is the honest, uniform mechanism (ADR-0007 rejected alternatives).
...
// of quiet approximation ADR-0002 forbids.
...
func (ps *projectSupervisor) restart(t supervisedService) error {
	...
	if out, err := ps.spawn(args...); err != nil {
		return fmt.Errorf("run: %v: %s", err, out)
	}
