...
struct TerminalSanitizer {
    ...
    private static func oscIsSafe(_ seq: [UInt8]) -> Bool {
        ...
        if code == 1337 {
            // iTerm2's file and variable channel.
            return false
        }
        if code == 52 {
            // Clipboard. A SET is how every TUI copies, and refusing it broke
            // copy/paste for real users; the worst it does is overwrite what
            // you were about to paste. A QUERY makes the terminal reply with
            // the clipboard's contents into the guest's stdin, which hands the
            // sandbox whatever the operator last copied, and stays refused.
            //
            // Shape: OSC 52 ; <selector> ; <data>, where "?" is the query.
            // Anything we cannot parse is treated as a query, because a set
            // wrongly refused costs a copy and a query wrongly allowed costs
            // the clipboard.
            guard fields.count >= 3 else { return false }
            let data = fields[fields.count - 1].trimmingCharacters(in: .whitespaces)
