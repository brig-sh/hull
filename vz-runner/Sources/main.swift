        ...
        while i < buf.count {
            ...
            if c == 0x1b {
                ...
                if let equivalent = TerminalSanitizer.c1Introducer(c) {
                    var alt: [UInt8] = [0x1b, equivalent]
                    alt.append(contentsOf: buf[(i + 1)...])
                    guard let n = TerminalSanitizer.escapeLength(alt, from: 0) else { break }
                    i += n - 1
                } else {
                    i += 1
                }
