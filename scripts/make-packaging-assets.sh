#!/bin/bash
#
# NOTE: the background caption needs an ImageMagick built with freetype. A
# build without it renders everything except the text, silently, with only a
# warning -- check the caption is there before shipping the result.
...
# The hull mark, not the urunc one: this artwork fronts hull's installer.
HULL_LOGO="${HULL_LOGO:-packaging/sources/hull-icon.png}"
OUT=packaging
TMP=$(mktemp -d)
trap 'rm -rf "$TMP"' EXIT
...
magick -size 1320x840 gradient:'#fbfbfd'-'#ebebf2' \
  ...
  -font /System/Library/Fonts/HelveticaNeue.ttc -pointsize 40 -fill '#3a3a42' -gravity north -annotate +0+290 'Drag hull to the Applications folder' \
