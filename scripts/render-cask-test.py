...
class RenderCaskTest(unittest.TestCase):
    ...
    def test_banner_names_the_repository_path(self):
        # channel.yml runs the renderer from a second checkout at .workflow/.
        with tempfile.TemporaryDirectory() as tmp:
            work = pathlib.Path(tmp)
            (work / ".workflow" / "scripts").mkdir(parents=True)
            (work / ".workflow" / "scripts" / "render-cask.py").write_bytes(RENDER.read_bytes())
            (work / "a.tar.gz").write_bytes(b"")
            subprocess.run([
                sys.executable, ".workflow/scripts/render-cask.py",
                "--project", "hull", "--version", VERSION, *CHANNEL,
                "--repo", "brig-sh/hull", "--tag", TAG, "--dist", ".",
                "--desc", "Run microVMs", "--archive=on_macos:on_arm:a.tar.gz",
                "--out", "cask.rb",
            ], cwd=work, check=True, capture_output=True)
            cask = (work / "cask.rb").read_text()
        self.assertIn("# Generated by scripts/render-cask.py in brig-sh/hull.", cask)
