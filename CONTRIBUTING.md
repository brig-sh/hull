...
- Conformance report: `docs/compose-conformance.md` is generated. After
  ...
  `make conformance-report` and commit the result; CI fails on a stale
  doc.
- End-to-end: the PTY and shared-folder harnesses under `test/`
  (`pty-terminal-test.py`, `pty-jobcontrol-test.py`, `share-test.py`)
  boot real VMs and need an Apple Silicon host with working HVF.
- Common commit scopes: `compose`, `run`, `exec`, `store`, `conformance`,
